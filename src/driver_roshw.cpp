...
class DriverROSHWNode : public rclcpp::Node
{
public:
  DriverROSHWNode(const rclcpp::NodeOptions & options = rclcpp::NodeOptions())
  : Node("driver_roshw", options)
  {
    ...
    sub_cmd_ = create_subscription<robot_msgs::msg::MotorCommandArray>(
      "motor_commands", 10, std::bind(&DriverROSHWNode::command_callback, this, _1));

    pub_imu_ = create_publisher<sensor_msgs::msg::Imu>("imu", 10);

    timer_ = create_wall_timer(
      std::chrono::milliseconds(10), std::bind(&DriverROSHWNode::publish_imu, this));
  }
...
int main(int argc, char ** argv)
{
  ...
  rclcpp::spin(std::make_shared<DriverROSHWNode>());
  rclcpp::shutdown();
  return 0;
}
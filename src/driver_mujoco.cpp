...
class DriverMujocoNode : public rclcpp::Node
{
public:
  DriverMujocoNode(const rclcpp::NodeOptions & options = rclcpp::NodeOptions())
  : Node("driver_mujoco", options)
  {
    ...
    sub_cmd_ = create_subscription<robot_msgs::msg::MotorCommandArray>(
      "motor_commands", 10, std::bind(&DriverMujocoNode::command_callback, this, _1));

    pub_imu_ = create_publisher<sensor_msgs::msg::Imu>("imu", 10);

    timer_ = create_wall_timer(
      std::chrono::milliseconds(10), std::bind(&DriverMujocoNode::publish_imu, this));
  }
...
int main(int argc, char ** argv)
{
  ...
  rclcpp::spin(std::make_shared<DriverMujocoNode>());
  rclcpp::shutdown();
  return 0;
}
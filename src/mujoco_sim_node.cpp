...
{
public:
  MujocoSimNode(const rclcpp::NodeOptions & options = rclcpp::NodeOptions())
  : Node("mujoco_sim_node", options)
  {
    ...
    sub_cmd_ = create_subscription<robot_msgs::msg::MotorCommandArray>(
      "motor_commands", 10, std::bind(&MujocoSimNode::command_callback, this, _1));

    pub_imu_ = create_publisher<sensor_msgs::msg::Imu>("imu", 10);

    timer_ = create_wall_timer(
      std::chrono::milliseconds(10), std::bind(&MujocoSimNode::publish_imu, this));
  }
